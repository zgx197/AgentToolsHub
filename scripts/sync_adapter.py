...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Synchronize a capability adapter from AgentToolsHub into a local agent skills directory."
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the resolved source and target without copying",
    )
...
def main() -> int:
    ...
    install_tree(source, target, force=True)
    print("Sync complete.")
    return 0
